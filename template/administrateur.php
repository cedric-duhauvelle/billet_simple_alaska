...
<div id="content_admin">
    ...
    <div id="content_form_admin">
        <form action="AdministrateurController" method="POST">
            ...
            <input id="chapter_titre" type="text" name="title" placeholder="Titre" <?php if (array_key_exists('id', $_GET)) {echo 'value="' . $chapter->displayTitle(filter_var($_GET['id'], FILTER_SANITIZE_STRING)) . '"';} ?> />
            <label class="chapter_admin" for="chapter_content">Contenu chapitre</label>
            <textarea id="chapter_content" name="chapter" placeholder="Ecrivez ici ..." alt="contenu chapitre"><?php if (array_key_exists('id', $_GET)){echo $chapter->displayContent(filter_var($_GET['id'], FILTER_SANITIZE_STRING));} ?></textarea>
